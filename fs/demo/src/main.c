...
static int demo_readdir(const char* path, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* fi)
{
    ...
    char buff[512];
    ddriver_read(super.driver_fd, buff, 512);
    
    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry temp;
    memcpy(&(temp.fname), buff, sizeof(struct demo_dentry));
    /* TODO: 填充filename */
    strcpy(filename, temp.fname);
...
static int demo_getattr(const char* path, struct stat *stbuf)
{
    if(strcmp(path, "/") == 0)
        ...
        stbuf->st_mode = S_IFREG | DEMO_DEFAULT_PERM;            // 该文件显示为普通文件
    return 0;
}
